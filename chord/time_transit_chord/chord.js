...
function updateChords( datasetURL ) {
    ...
    groupG.select("title")
        .text(function(d, i) {
            return numberWithCommas(d.value**2) 
                + " papers relates to " 
                + regions[i].name;
        });
    ...
    chordPaths.select("title")
        .text(function(d) {
            if (regions[d.target.index].name !== regions[d.source.index].name) {
                return [numberWithCommas(Math.round(2**d.source.value)+Math.round(2**d.target.value)),
                        " papers relates to ",
                        regions[d.source.index].name,
                        " and ",
                        regions[d.target.index].name
                        // "\n",
                        // numberWithCommas(Math.round(2**d.target.value)),
                        // " exports from ",
                        // regions[d.target.index].name,
                        // " to ",
                        // regions[d.source.index].name
                ...
                return numberWithCommas(Math.round(2**d.source.value)) 
                    + " exports ended in " 
                    + regions[d.source.index].name;
            }
        });
...
/* Activate the buttons and  to data sets */
