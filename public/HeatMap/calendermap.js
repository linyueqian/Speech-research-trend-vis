...
var color = d3.scale.linear().range(["white", 'darkred'])
    .domain([0, 8.9]);
...
d3.csv("data3.csv", function(error, csv) {
  ...
  var data = d3.nest()
    ...
    .rollup(function(d) { return (d[0].Comparison_Type) ; })//Math.sqrt(d[0].Comparison_Type / Comparison_Type_Max)  d[0].Comparison_Type
    .map(csv);

	globalData = data;
  rect.filter(function(d) { return d in data; })
      .attr("fill", function(d) { return color(data[d]); })
	  .attr("data-title", function(d) { return "value : " + Math.round(2**data[d])})
    .on('mouseover', function(d, i, j) {
      d3.select('#colLabel_' + i).classed("hover", true);
      d3.select('#rowLabel_' + j).classed("hover", true);
      if (d != null) {
          let s = dateFromDay(2020+j, i+1)
          tooltip.html('<div class="heatmap_tooltip">' + s + '<br>' + (Math.round(2**data[d])-1) + " papers" + '</div>');
      ...
      .on("mousemove", function(d, i) {
          tooltip.style("top", (d3.event.pageY - 80) + "px").style("left", (d3.event.pageX - 100) + "px");
      })
      .on('click', function() {
...
function changePalette(paletteName, globalData) {
  ...
  switch(paletteName) {
    case "Cat20":
        colorScale = d3.scaleLinear(d3.interpolateBuGn)
        .domain([0, 8.91]);
        break;
    case "Cat20b":
        colorScale = d3.scale.linear().range(["white", '#002b53']).domain([0, 9]);
        break;
    case "Cat20c":
        colorScale = d3.scale.linear().range(["white", '#002b53']).domain([0, 9]);
        break;
    default:
        ...
        var colorScale = d3.scale.linear().range(["white", '#002b53']).domain([0, 9]);
        break;
  }
